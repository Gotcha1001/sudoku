...
//   { label: "Play Locally", path: "/local", icon: MonitorSpeaker, badge: "🏠" }, // ← NEW
...
const NAV_ITEMS = [
  { label: "Dashboard", path: "/dashboard", icon: Home },
  { label: "Play", path: "/play", icon: Gamepad2, badge: "🧩" },
  { label: "Game History", path: "/history", icon: History },
  { label: "Leaderboard", path: "/leaderboard", icon: Trophy, badge: "🔥" },
  { label: "Settings", path: "/settings", icon: Settings },
];
...
export function AppSidebar() {
  ...
  return (
    <Sidebar className="relative w-64 bg-white dark:bg-slate-950">
      {/* Animated dark background (dark mode only) */}
      <div className="hidden dark:block absolute inset-0 z-0 overflow-hidden">
        <motion.div
          className="absolute top-[-200px] left-[-200px] w-[600px] h-[600px] rounded-full bg-blue-900 opacity-50"
          animate={{ scale: [1, 1.3, 1], x: [0, 50, 0], y: [0, -50, 0] }}
          transition={{ duration: 20, repeat: Infinity, repeatType: "mirror" }}
        />
        <motion.div
          className="absolute bottom-[-250px] right-[-250px] w-[700px] h-[700px] rounded-full bg-slate-950 opacity-50"
          animate={{ scale: [1, 1.25, 1], x: [0, -50, 0], y: [0, 50, 0] }}
          transition={{ duration: 25, repeat: Infinity, repeatType: "mirror" }}
        />
      </div>
      ...
      {/* Header */}
      <SidebarHeader className="border-b border-gray-200 dark:border-blue-800 p-4 relative z-10">
        <Link href="/" className="flex items-center gap-2">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-600 dark:bg-blue-700 shadow-lg">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-gray-500 dark:text-blue-300 uppercase tracking-widest">
              Logic Puzzle Game
            </span>
          </div>
        </Link>
      ...
      <SidebarContent className="relative z-10">
        <SidebarGroup>
          <SidebarGroupLabel className="text-gray-500 dark:text-blue-300">
            Navigation
          </SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {NAV_ITEMS.map(({ label, path, icon: Icon, badge }) => (
                <SidebarMenuItem key={path}>
                  <SidebarMenuButton
                    onClick={() => handleNav(path)}
                    isActive={pathname === path}
                    className="cursor-pointer text-gray-700 dark:text-blue-200 hover:text-blue-600 dark:hover:text-blue-300 rounded-md"
                  >
                    <Icon className="h-4 w-4" />
                    <span className="flex-1">{label}</span>
                    {badge && (
      ...
      {/* Footer */}
      <SidebarFooter className="border-t border-gray-200 dark:border-blue-800 p-4 relative z-10">
        <div className="flex items-center justify-between">
